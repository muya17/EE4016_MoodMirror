...
from models import SimpleCNN, LiteCNN, CLCM, count_parameters
...
def format_time(seconds):
    """Helper function to format seconds into readable time"""  # ← NEW
    if seconds < 60:
...
def train_model(model_class, model_name):
    """
    Main training loop for SimpleCNN and LiteCNN architectures.
    Features: MPS/CUDA acceleration, LR scheduling, and best-model checkpointing.
    
    :param model_class: The model architecture class (SimpleCNN or LiteCNN)
    :param model_name: String name for saving and logging purposes
    ...
    print(f"\nInitializing {model_name} Architecture...")
    ...
    # This is CRITICAL for pushing accuracy past baseline towards the 68%/70% target
    ...
    training_start = time.time()  # ← NEW: Track overall start time
    
    # Create tqdm progress bar for epochs  ← NEW
    ...
    for epoch in epoch_pbar:
        ...
        # --- Calculate Timing & Remaining Time ---  ← NEW
        ...
        # --- Update Progress Bar with Metrics ---  ← NEW
    ...
    epoch_pbar.close()  # ← NEW: Close progress bar
...
def evaluate_model(model_class, model_path, model_name):
    ...
    print("\n" + "="*70)
    print(f"🎯 TEST ACCURACY FOR {model_name.upper()}: {final_test_acc:.2f}%")
    print("="*70 + "\n")
...
if __name__ == '__main__':
    ...
    print("="*70)
    simple_acc = train_model(SimpleCNN, "SimpleCNN")
    ...
    # Train LiteCNN (Target: ≥70%, <1.5M params, Section 4.4, O3)
    print("\n" + "="*70)
    print("PHASE 2: TRAINING LITECNN")
    print("Target: ≥70% accuracy, <1.5M parameters, <10ms CPU (Section 4.4, O3)")
    print("="*70)
    lite_acc = train_model(LiteCNN, "LiteCNN")

    lite_test_acc = evaluate_model(
        LiteCNN, 
        os.path.join(SAVE_DIR, 'LiteCNN_best.pth'), 
        "LiteCNN"
    )
    ...
    print()
    print(f"LiteCNN Validation Accuracy:   {lite_acc:.2f}% (Target: ≥70%)")  # ← Added
    print(f"LiteCNN Test Accuracy:         {lite_test_acc:.2f}%")            # ← Added
    print()
    ...
    if lite_acc >= 70.0:
        ...
        print("⚠️  LiteCNN BELOW TARGET - Consider more epochs or augmentation")
