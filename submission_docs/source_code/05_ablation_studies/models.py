...
class SimpleCNN(nn.Module):
    """Simple CNN baseline model with configurable dropout rate."""

    def __init__(self, dropout_rate=0.5, num_classes=7):
        """
        Args:
            dropout_rate (float): Dropout rate for the final fully connected layer.
        ...
        self.dropout = nn.Dropout(0.25)          # fixed intermediate dropout
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout_fc = nn.Dropout(dropout_rate)   # configurable
...
class LiteCNN(nn.Module):
    """LiteCNN target model with residual depthwise blocks and configurable dropout."""

    def __init__(self, dropout_rate=0.3, num_classes=7):
        """
        Args:
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(dropout_rate),   # configurable
